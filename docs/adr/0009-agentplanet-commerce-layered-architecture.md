...
| C5 | P1-A delivery shape: full AP2 `PaymentTask` (created+confirmed) vs lightweight "notify seller webhook" call | **Decided: full AP2 task.** Backend, after settling in its ledger, drives a `platform_credits` task created **already-confirmed** via an internal endpoint (settlement already happened — no buyer `tx_hash` dance); ACN fires the seller webhook. Task is an event/coordination mirror, never the money authority. |
| C6 | Per-agent webhook secret model | **Decided: recoverable shared secret, stored consistent with existing ACN practice.** Webhook signing is a symmetric secret ACN uses to sign **outgoing** deliveries, so it must be recoverable (**NOT hashed**). ACN has no app-level encryption facility today and deliberately stores `agents.api_key` as plaintext behind infra-level (Railway/DB) at-rest encryption — so store the webhook secret the **same way** (recoverable, infra-encrypted); do **not** introduce a new Fernet/KMS layer just for this. Generate at capability registration, return once for the seller's verifier, allow rotation. Sign HMAC-SHA256 over raw body → `X-ACN-Signature: sha256=...`; include `X-ACN-Timestamp`, seller rejects stale timestamps (replay) + constant-time compare. |
...
| C8 | AP2 task lifecycle completeness | **Decided: advance task to `task_completed` on seller fulfill.** A store settlement jumps the task to `payment_confirmed`; without a completion step the task lingers there forever and pollutes ACN task queries/metrics. On `fulfill`, the backend advances the mirror task to `task_completed` (fires `payment_task.completed`). Keeps ACN task state consistent with the store order. |
