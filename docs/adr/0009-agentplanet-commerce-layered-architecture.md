...
| C6 | Per-agent webhook secret model | **Decided: encrypted-at-rest shared secret** (NOT hashed). Webhook signing is a symmetric secret ACN uses to sign **outgoing** deliveries, so ACN must store it recoverably (encrypt at rest); generate at capability registration, return once for the seller's verifier, allow rotation. Sign HMAC-SHA256 over raw body → `X-ACN-Signature: sha256=...`; include `X-ACN-Timestamp` and have the seller reject stale timestamps (replay) + constant-time compare. |
