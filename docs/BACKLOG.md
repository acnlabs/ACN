...
| 11b | `websocket-protocol`                 | RFC approved 2026-05-03 (`docs/features/acn-error-schema-websocket.md`). Decisions: D1c hybrid channel (close-reason JSON for handshake-phase + application error-frame for post-handshake) · D2b RFC-mapped close codes (4400/4401/4403/4429/1011) · D3a reuse cross-module catalog (no new ErrorCodes; 3 new `AUTHENTICATION_REQUIRED.details.reason` values: `ws_query_token_disabled` / `ws_invalid_auth_message` / `ws_invalid_auth_message_format`) · Q3 distinct close codes 4401 / 4403 for site #4's two halves (matches HTTP #11a precedent, prevents transport-switch enumeration oracle) · Q4 SDK 0.6.0 major break with 30-day bake window. Implementation entry: `acn/routes/websocket.py` `_safe_close` helper signature extension + 4 site migrations + new contract test file `tests/routes/test_websocket_error_schema_protocol.py` + AST consistency walker extension to `_safe_close`. | 🟡 RFC approved, ready for implementation |
