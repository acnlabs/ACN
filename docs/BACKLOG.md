...
| 4-followup | `tasks` (auth/permission/validation) | depends on #2b auth code naming — 10× 403 `PermissionError` re-raises (ownership, one per write endpoint going through `task_service`), 10× 400 `ValueError` re-raises (body / status validation, paired 1:1 with PermissionError sites bar two), 6× endpoint-specific 4xx (1× 401 invalid agent key + 1× 403 missing `acn:write` JWT perm in `require_task_write_auth`, 1× 400 invalid status enum in `list_tasks`, 1× 400 empty tag list in `match_tasks_for_agent`, 2× 403 on `get_task`'s private-task subnet gate) | ⏳ |
