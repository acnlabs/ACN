...
| 11b | `websocket-protocol`                 | Implemented 2026-05-03 (commit `76afef5`). All 5 handshake-phase error sites in `WEBSOCKET /ws/{agent_id}` migrated to typed dual-channel contract (`acn/docs/features/acn-error-schema.md` §5 → "WebSocket protocol close-frame contract", and the full RFC at `acn-error-schema-websocket.md`). 0 new ErrorCodes (D3a — reused `AUTHENTICATION_REQUIRED` and `API_KEY_AGENT_MISMATCH` byte-identical to HTTP routes); 3 new `AUTHENTICATION_REQUIRED.details.reason` values (`ws_query_token_disabled`, `ws_invalid_auth_message`, `ws_invalid_auth_message_format`); site #4 split into 4a (`AUTHENTICATION_REQUIRED` close 4401) and 4b (`API_KEY_AGENT_MISMATCH` close 4403) per Q3 (driver: SDK ergonomics — both transports produce the same typed `error_code` for the same logical failure; not a security oracle closure since HTTP already discriminates the two). **Post-implementation revision**: D1c hybrid channel was revised to **D1b uniform** during implementation — `api_key_agent_mismatch`'s two-UUID `{path_agent, key_agent}` payload overflows RFC 6455's 123-byte close-reason budget by ~60 bytes, so `details` moved exclusively onto the application error-frame channel; close-reason carries only `{c, r}` as a typed-but-minimal fallback. New `_send_error_and_close` combo helper sends both channels uniformly. New `WEBSOCKET_CLOSE_REASON_FORMAT` config flag (`legacy` default during 30-day SDK 0.6.0 bake window, `compact` post-bake — target flip 2026-06-15). New 6-test contract file `tests/routes/test_websocket_error_schema_protocol.py` (1:1 site coverage + cross-cutting byte-budget guard). AST consistency walker extended to `_send_error_and_close` calls so cross-channel `details` drift is statically caught (18 emitters of `api_key_agent_mismatch` now tracked across both emitter forms — 17 HTTP + 1 WS). 16 existing M14 security tests continue to pass under bake-window default; 2 test names + close-code assertions updated to reflect the new RFC-mapped 4400/4403 codes. | ✅ Aligned (#11b, behind feature flag during bake) |
