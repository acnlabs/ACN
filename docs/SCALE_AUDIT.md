...
### [ ] P0-1 任务删除未清理 participation 全量键族（确定性泄漏）
...
### [ ] P0-2 PostgreSQL 删任务不清理仍用于索引的 Redis 键（混合部署泄漏）
...
### [ ] P0-3 审计「按日」索引 list 有 TTL 但无条数上限（单日撑爆单 key）
...
### [ ] P0-4 心跳兜底 `mark_offline_stale` 全量加载 Agent
