...
**Status**: ✅ Implemented (2026-05-03; bake-window flag removed 2026-05-04 — compact is the only wire shape)
...
- **Q3 (resolved: distinct codes 4401 / 4403)** — Site #4 splits cleanly: 4401 + `AUTHENTICATION_REQUIRED reason=invalid_api_key` for the token-not-found half (caller's key did not resolve to any agent), 4403 + `API_KEY_AGENT_MISMATCH` for the token-resolves-to-wrong-agent half. This matches the HTTP `/status` endpoint behaviour migrated under #11a so SDK 0.6.0+ clients can build one `error_code → domain exception` mapping that works identically for both transports. **Note (post-implementation correction)**: the original RFC framed this as "preventing a transport-switch enumeration oracle" — that framing is incorrect, since the HTTP route already discriminates 401/403 on its own and a pure-HTTP attacker can already enumerate "key bad" vs "key for wrong agent" without touching WS. The WS split is **parity-with-HTTP** for SDK ergonomics, not a confidentiality improvement. The genuine confidentiality lever is the policy decision (echoed by both transports) to surface `key_agent` in the response — closing that requires changing both transports together.
...
- Reviewers ack §3 recommendations (D1c, D2b, D3a) — all accepted as-drafted.
- Open questions §7-Q1..Q5 each resolved (defaults accepted, no overrides).
- SDK release-note plan agreed — Q4 → SDK 0.6.0 (major), 30-day bake with deprecation warning helper.
- BACKLOG.md row #11b flipped from "RFC required" to "ready for implementation" with the agreed scope.
