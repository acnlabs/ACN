...
**Status**: ✅ All sprints landed — Phase 2 review v2 P1 #11 (sprints #1–#11b, May 2026). HTTP routes and the WebSocket protocol endpoint both speak the typed schema. Legacy bake-window code removed 2026-05-04; compact wire shape is now the only mode (`WEBSOCKET_CLOSE_REASON_FORMAT` flag deleted). 5xx `error` field removed same date. Phase 3 Module B — six new `attention_fee_*` codes added 2026-05-05 for the lock → ack → release pipeline (catalog stays append-only).
...
### ~~5xx deprecation double-emit~~ (removed 2026-05-04)

The 5xx body briefly carried a legacy `error` field equal in value to `error_code` during the migration window. **It has been removed** — the 5xx body now uses the same shape as 4xx:
...
Tracker: see "Phase 2 review v2 P1 #11 — Error schema migration sprint" in `[docs/BACKLOG.md](../BACKLOG.md)` for the deprecation history (removed early because no real external SDK clients existed).
...
| `communication_rejected` | 403                                 | `/send` `/internal/send` (defensive); registry catch-all proxy (`POST/PUT/PATCH /{agent_id}{/rest_path}`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `{ reason: string, reject_reason: string           |
...
### Attention fee (Phase 3 Module B — 2026-05-05)

Phase 3 economic-model field. Sender attaches `attention_fee` on
`POST /communication/send`; the ACN gateway locks Credits in Backend
Escrow before writing the manifest entry; receiver releases via
`POST /communication/manifest/{agent_id}/{mid}/ack`. Six error codes
cover the lock → ack → release pipeline, each mapping to a *distinct*
caller-actionable failure mode at a different stage.

| `error_code`                            | HTTP status | Used by                                                                                                                                                                                                       | `details` schema                                                |
| --------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| `attention_fee_invalid`                 | 400         | `POST /api/v1/communication/send` — `attention_fee.currency` not in the supported set (`{credits}` today). Pydantic-side amount/range violations surface as 422 *before* this code can fire.                  | `{ currency: string, supported: string[] }`                     |
| `attention_fee_requires_manifest_mode`  | 400         | `POST /api/v1/communication/send` — `attention_fee` was supplied but the recipient's `communication_policy` routes the message to inbox / rejection. No escrow lock was created; sender funds are untouched. | `{ recipient_id: string, actual_route: string }`                |
| `attention_fee_lock_failed`             | 400         | `POST /api/v1/communication/send` — Backend Escrow `lock_v2` rejected the lock (insufficient sender balance, idempotency collision, wallet missing). No manifest entry was written.                           | `{ reason: string }` (Backend's failure reason, verbatim)       |
| `attention_fee_not_locked`              | 400         | `POST /api/v1/communication/manifest/{agent_id}/{mid}/ack` — entry exists for the caller, but no `attention_fee` was attached at send time. Caller should use `GET /communication/content` instead.           | `{ agent_id: string, mid: string }`                             |
| `attention_fee_already_acked`           | 400         | `POST /api/v1/communication/manifest/{agent_id}/{mid}/ack` — replay/double-ack on an already-released fee. Idempotent-safe on the recipient side; SDK clients should suppress duplicate ack calls.            | `{ agent_id: string, mid: string }`                             |
| `attention_fee_release_failed`          | 400 / 503   | `POST /api/v1/communication/manifest/{agent_id}/{mid}/ack` — Backend Escrow `release_partial` rejected the release (escrow vanished, refunded out from under us, service down). `acked_at` is rolled back so the SDK can retry. **Also** emitted by `DELETE /api/v1/communication/manifest/{agent_id}/{mid}` on entries with a locked `attention_fee` when Backend Escrow `refund_v2` rejects the refund — the manifest row is preserved (no orphan). The 503 variant fires from the DELETE path when the deployment runs with `ESCROW_ENABLED=false` and the entry has a locked fee. | `{ agent_id: string, mid: string, reason: string, operation?: "release" \| "refund" }` |

**Why six distinct codes (vs a single `attention_fee_failure` with `details.kind`).** Each code maps to a *materially different* SDK reaction:

* `_invalid` / `_requires_manifest_mode` — fix the request shape and retry.
* `_lock_failed` — top up the wallet, then retry.
* `_not_locked` — recipient-side: call `GET /communication/content` instead, ack is the wrong endpoint.
* `_already_acked` — recipient-side: idempotent no-op, suppress duplicate calls.
* `_release_failed` — recipient-side: backend transient, retry with backoff. The `details.operation` field (`"release"` for ack path, `"refund"` for DELETE path) tells the SDK which lifecycle stage failed without forcing it to inspect the raw `reason` string.

Collapsing them would force every SDK to inspect `details` to recover the same branching logic, defeating the flat-code contract.

**No 5xx exposure.** The lock and release paths catch `httpx.RequestError` inside `EscrowClient` and return `success=False` — those flow into 4xx `_lock_failed` / `_release_failed` with the underlying error in `details.reason`. We do not surface 5xx for backend hiccups because the caller can meaningfully retry; the central 5xx sanitiser only sees these errors if the route handler itself crashes (e.g. Redis down → manifest read fails before the lock path). 5xx in that case is correct: the caller's retry is no better than the next caller's, and surfacing the underlying error would leak infrastructure detail.

**Backwards-compat.** Pre-Phase-3 SDK clients omit the `attention_fee` field entirely and continue to receive identical 200 responses with no escrow side effects (`tests/routes/test_communication_attention_fee.py::TestBackwardsCompat`).

The 25 contract tests in `tests/services/test_attention_fee.py`, `tests/routes/test_attention_fee_routes.py`, and `tests/routes/test_communication_attention_fee.py` pin every raise site (1:1 coverage) plus the HSETNX-based ack idempotency, escrow lock-before-write ordering, and acked_at rollback on release failure.
...
| ------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
| `authentication_required` | 401         | `GET /api/v1/analytics/activities` — when an `agent_id` or `agent_ids` filter is requested without (or with a malformed) `Authorization: Bearer …` header; or with a Bearer token that does not resolve.     | `{ reason: "auth_required_for_agent_filter"                                |
...
- `authentication_required` — `"invalid_authorization_header_format"` (registry `GET /me`; **dependencies** `verify_agent_api_key` + `verify_proxy_caller` — same reason value used across both header names because the SDK-actionable failure is identical), `"invalid_api_key"` (registry; **dependencies** `_resolve_agent_by_bearer`; **analytics** `list_activities` Bearer-key-fails-to-resolve branch; **websocket protocol** `WEBSOCKET /ws/{agent_id}` resolved-key-not-found branch — same reason value across all four emitters because the SDK-actionable failure is identical), `"owner_filter_requires_auth"` (subnets), `"private_subnet"` (subnets), `"invalid_agent_api_key"` (tasks), `"owner_or_internal_credential_required"` (**dependencies** `verify_owner_or_internal` no-credential branch), `"auth_required_for_agent_filter"` (**analytics** `list_activities` no-Bearer branch — distinct from `invalid_api_key` so SDK clients can tell "you must authenticate to filter" apart from "your auth failed"), `"ws_query_token_disabled"` / `"ws_invalid_auth_message"` / `"ws_invalid_auth_message_format"` (**websocket protocol** sprint #11b — three handshake-phase failure modes; `ws`_* prefix marks them as WS-only so an HTTP-side parser ignoring WS reasons does not need a dedicated allow-list)
...
The **WebSocket protocol endpoint** (`WEBSOCKET /ws/{agent_id}`) is also aligned, via sprint #11b. Because RFC 6455 close frames cannot carry an HTTP body, #11b uses a typed dual-channel contract — application error-frame on the WS data channel + RFC-mapped close code with a compact `{c, r}` close-reason fallback. The bake-window flag has been removed (2026-05-04); compact is the only wire shape. See [§5 → "WebSocket protocol close-frame contract"](#websocket-protocol-close-frame-contract-sprint-11b) for the SDK parsing template, and `[acn-error-schema-websocket.md](acn-error-schema-websocket.md)` for the full migration RFC.
...
*Bake-window flag* — The `WEBSOCKET_CLOSE_REASON_FORMAT` config flag and legacy code branch were removed 2026-05-04 (no real external SDK 0.5.x clients to protect). Compact is now the only wire shape. The original design rationale is preserved in `acn-error-schema-websocket.md` §Q4 for reference.
...
**The M14 security tests** (`tests/routes/test_websocket_auth_m14.py`) were updated for the compact-only path: they now consume the application error-frame before asserting the `WebSocketDisconnect` close code. The security invariants (`ws_manager.connect.assert_not_called()` for failed-auth paths) are unchanged. The dedicated contract tests in `test_websocket_error_schema_protocol.py` pin the dual-channel wire shape.
```
...
The compact wire shape (application error-frame + `{c, r}` close-reason) is now the only mode. The bake-window flag (`WEBSOCKET_CLOSE_REASON_FORMAT`) and legacy code path were removed 2026-05-04.
