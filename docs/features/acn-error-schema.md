...
The route module `websocket` still raises vanilla `HTTPException` and is caught by the existing `_http_exception_handler` 4xx pass-through, which emits the legacy `{"detail": "..."}` / `{"detail": {...}}` shape. The `registry`, `subnets`, `tasks`, `payments`, `follows`, `manifest`, `dependencies`, `analytics`, and `onchain` modules are fully migrated as of sprints #2b, #3-followup, #4-followup, #5, #6, #8, #10, #9, and #7 respectively. See the coexistence matrix below. The websocket surface is NOT broken, it just speaks the old contract until sprint #11 — note that the websocket protocol's error contract is bounded by RFC 6455 close codes, so its migration will require separate RFC / design work distinct from the HTTP-only schema convergence done in sprints #1-#10.
