...
The remaining 26 4xx sites in `acn/routes/tasks.py` need new catalog codes — 10× `PermissionError` re-raises (403), 10× `ValueError` re-raises (400 — body / status validation), and 6 endpoint-specific 4xx (auth gates on `require_task_write_auth`, status / tag body validation in `list_tasks` / `match_tasks_for_agent`, subnet-membership enforcement on private tasks). See footnote `[^4]` below for the precise enumeration. They land alongside sprint rows #2b/#2c once the auth/permission codes settle on names — the ownership/permission semantics overlap heavily with registry's deferred set, so reusing the same vocabulary keeps the catalog small.
...
| ------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| `authentication_required` | 401         | **registry** — `GET /me` (×2: invalid Authorization header format / unrecognised API key); **subnets** — `GET /subnets?owner=…` (owner-filter requires auth), `GET /subnets/{id}/agents` (private subnet requires auth)                                                                                                                                                                     | `{ reason: enum, subnet_id?: string }`                                 |
| `internal_token_invalid`  | 401         | **registry** — `POST /agents/join/internal` (X-Internal-Token missing or mismatched)                                                                                                                                                                                                                                                                                                       | `{}`                                                                   |
| `missing_permission`      | 403         | **registry** — `POST /agents/dev/register` (dev-mode disabled in this environment)                                                                                                                                                                                                                                                                                                         | `{ reason: enum }`                                                     |
| `ownership_mismatch`      | 403         | **registry** — `POST /register-protected` (owner-token mismatch); `DELETE /agents/{id}` `POST /claim` `POST /transfer` `POST /release` (`PermissionError` re-raises); **subnets** — `GET /subnets?owner=…` (cross-tenant non-admin), `DELETE /subnets/{id}` (`PermissionError` re-raise)                                                                                                    | `{ agent_id?: string, subnet_id?: string, reason?: string } \| { requested_owner: string, token_owner: string }` |
| `not_subnet_member`       | 403         | **subnets** — `GET /subnets/{id}/agents` against a private subnet by a non-owner non-admin caller                                                                                                                                                                                                                                                                                          | `{ subnet_id: string, agent_id: string }`                              |
| `invalid_request`         | 400         | **registry** — `DELETE /api/v1/agents` (bulk-delete filter required), `POST /agents/{id}/claim` (`ValueError` from claim flow); **subnets** — `POST /subnets` (`ValueError` from create flow)                                                                                                                                                                                              | `{ reason: string, agent_id?: string }`                                |

`details.reason` is a stable per-code enum — it gives the SDK a way to branch on the cause without parsing the prose `message`. Reason values currently emitted, by `error_code`:

* `authentication_required` — `"invalid_authorization_header_format"` (registry), `"invalid_api_key"` (registry), `"owner_filter_requires_auth"` (subnets), `"private_subnet"` (subnets)
* `missing_permission` — `"dev_mode_disabled"` (registry)
* `invalid_request` — `"bulk_delete_filter_required"` (registry); free-form `str(ValueError)` (registry claim path, subnets create path) — these wrap the underlying `ValueError` message verbatim
* `ownership_mismatch` — free-form `str(PermissionError)` from the underlying service (registry / subnets); the registry owner-token mismatch path uses no reason (the field set itself — `requested_owner` + `token_owner` — already disambiguates)
...
The route modules `follows`, `payments`, `onchain`, `dependencies`, `manifest`, `analytics`, and `websocket` still raise vanilla `HTTPException` and are caught by the existing `_http_exception_handler` 4xx pass-through, which emits the legacy `{"detail": "..."}` / `{"detail": {...}}` shape. The `registry` and `subnets` modules are fully migrated as of sprint #2b and #3-followup; `tasks` is partially migrated as of sprint row #4 (its followup migration lands alongside the next commit). See the coexistence matrix below. These routes are NOT broken, they just speak the old contract until the migration sprint reaches them.
...
[^4]: **Tasks partial migration (sprint #4).** 13 of the 39 4xx raise sites in `acn/routes/tasks.py` are migrated — every `except TaskNotFoundException: raise … from None` site (`GET /tasks/{id}`, `POST /tasks/{id}/{accept,invite,submit,review,cancel}`, `GET /tasks/{id}/participations`, the three `POST /tasks/{id}/participations/{pid}/{cancel,approve,reject}`, `GET /tasks/{id}/internal`, `POST /tasks/agent/{id}/{accept,submit}`). All 13 use the same uniform shape `ACNHTTPError(TASK_NOT_FOUND, 404, {"task_id": …}) from None`, making this the most uniform sprint of the migration so far. The remaining 26 4xx sites need new catalog codes — 10× `PermissionError` re-raises (403, ownership semantics overlapping registry's deferred set, one per write endpoint that goes through `task_service`), 10× `ValueError` re-raises (400, body / status validation, paired 1:1 with the 10 `PermissionError` sites except for `list_participations` and the dedicated `acn:write` JWT permission gate), and 6 endpoint-specific 4xx — 1× 401 *invalid agent API key* (`require_task_write_auth` agent path, `tasks.py:94`), 1× 403 *missing `acn:write` permission* (JWT path, `tasks.py:107`), 1× 400 *invalid status enum* (`list_tasks` filter, `tasks.py:510`), 1× 400 *tag list empty* (`match_tasks_for_agent` body, `tasks.py:554`), 2× 403 on `get_task`'s private-task subnet-membership gate (`tasks.py:589` *authentication required to view this task* + `tasks.py:592` *not a subnet member*). They pick up alongside sprint rows #2b/#2c once the auth / permission codes settle on names. The 1 5xx site (`create_task` catch-all) stays on `HTTPException` by design (sanitised-5xx handler chain). Sprint row #4 flips fully ✅ when the auth / permission / validation codes land.
