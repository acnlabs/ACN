...
`task_not_found` / `wallet_rate_limit_exceeded` / `authentication_required` / `internal_token_invalid` / `insufficient_balance` / `resource_conflict`
...
The route modules `tasks`, `follows`, `payments`, `onchain`, `dependencies`, `manifest`, `analytics`, and `websocket` still raise vanilla `HTTPException` and are caught by the existing `_http_exception_handler` 4xx pass-through, which emits the legacy `{"detail": "..."}` / `{"detail": {...}}` shape. The `registry` and `subnets` modules are partially migrated as of sprint rows #2a and #3 respectively — see the coexistence matrix below. These routes are NOT broken, they just speak the old contract until the migration sprint reaches them.
