...
The route modules `onchain` and `websocket` still raise vanilla `HTTPException` and are caught by the existing `_http_exception_handler` 4xx pass-through, which emits the legacy `{"detail": "..."}` / `{"detail": {...}}` shape. The `registry`, `subnets`, `tasks`, `payments`, `follows`, `manifest`, `dependencies`, and `analytics` modules are fully migrated as of sprints #2b, #3-followup, #4-followup, #5, #6, #8, #10, and #9 respectively. See the coexistence matrix below. These routes are NOT broken, they just speak the old contract until the migration sprint reaches them.
