...
- Mode B `--chat-writeback` forwards complete JSON `tool_lines`
  (`kind:image` only, units 1–100). Host occupies per still in this chat;
  dislike is not a refund; no goods → no occupy. Restart `acn listen`
  after upgrading.
...
- Interfaze skill install URL is the public Host API
  `https://api.agentplanet.org/api/interfaze/skill.md` (Host git is private;
  GitHub raw 404). Scripts: `/api/interfaze/scripts`.
...
- Official hops with `--chat-complete-exec` / `--chat-complete-url` go
  back to the agent (Host door + `OPENAI_BASE_URL`). Host must have
  seen the hop or writeback fails. No complete source still uses the
  CLI Host POST.

## [1.0.11] - 2026-08-23

CLI-only patch (`@acnlabs/acn-cli@1.0.11`). Server / SDKs stay **1.0.1**.
Skill stays **1.0.6**.

### Changed

- Official hop door forwards Host SSE instead of buffering the whole
  body. CLI in-process official complete stays non-stream.
