...
- **Subnets carry a `join_policy` field** (`open` / `approval`). `is_private=True` ⇒ `join_policy='approval'`; the `private + open` combination is rejected at entity construction with `visibility_policy_conflict`. Phase 1 populates the field through the create-path (service infers `approval` when callers omit it on private subnets, preserving backward compat); the admission gate that reads `join_policy` on the join path — request / invitation / allowlist state machine — lands in Phase 2. See `docs/adr/0004-subnet-join-policy.md`.
