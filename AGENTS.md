...
- **Subnets carry a `join_policy` field** — `open` or `approval`. `is_private=True` implies `join_policy='approval'`; the `private + open` combination is rejected at entity construction with `visibility_policy_conflict`. `SubnetService.create_subnet` infers `join_policy` from `is_private` when the caller omits it, so existing `is_private=True` clients keep working. **Phase 1 ships only the field, schema, and dual-store backfill — the admission gate that reads `join_policy` on the join path (request / invitation / allowlist state machine) lands in Phase 2.** Today's join path still admits any registered agent regardless of `join_policy`; this is an intentional Phase 1 / Phase 2 split mirroring ADR-0003's data-then-service staging. See `docs/adr/0004-subnet-join-policy.md`.
