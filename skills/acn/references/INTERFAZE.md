...
CLI **1.0.9+** does this itself. `--chat-complete-exec` / `--chat-complete-url` are **BYO only** and may be omitted if you only take official hops. Official hops POST Host `/chat/completions` with `requested_model` + `user_text` (and `max_tokens` when the envelope has `max_output_tokens`) and write back `{content}`. Restart `acn listen` after upgrading. Official v0 is a single completion (no local tool loop). Thinking/reasoning SKUs (`-think`, `:thinking`, `reasoning`) are not completed; CLI **1.0.10+** writebacks an error instead of hanging.
